...
def guidedBackPropTest(model, image_path, k = 1):
...
def gradCamTest(model, image_path, k = 1, layerList = ['layer4'], counterFactual = False):
...
def guidedGradCamTest(model, image_path, k = 1, layerList = ['layer4']):
...
def runExperiment(layerList = ["features"], image_path = "cat_dog.png", model = getVGGModel(16), k=1, counterFactual = False, experiment_name = "GradCam"):
    ...
    if experiment_name == "GuidedBp":
        guidedBackPropTest(model,image_path, k)
    elif experiment_name == "GradCam":
        gradCamTest(model,image_path,k,layerList, counterFactual)
    else:
        guidedGradCamTest(model,image_path,k, layerList)
